...
# ── 5. PBAC cycle calendar ────────────────────────────────────────────────────

def load_pbac_calendar() -> dict:
    """Load data/pbac_calendar.json (produced by parse_pbac_calendar.py).

    Returns a dict with 'meetings' (sorted by meeting date), plus a
    'last_milestone' date so the dashboard can warn when the calendar is stale.
    """
    path = _d("pbac_calendar.json")
    if not path.exists():
    ...
    # Find the latest milestone date across all meetings (used to flag staleness)
    all_dates = [
        dl["date"]
        for m in meetings
